...
| Default Codex home derives from `$CODEX_HOME` or the standard `$HOME/.codex`; tests use isolated temporary paths. | `src/main.rs` (`default_codex_home`) | Portable for the supported Unix platforms; a missing home fails explicitly. | Keep provider home semantics capability-specific when more backends are added. |
